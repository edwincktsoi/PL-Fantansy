...
def fetch_and_forecast_players():
    ...
    teams_df = pd.DataFrame(bootstrap['teams']) # <-- ADDED: Fetch teams data
    
    # --- NEW: Create Team Name Map ---
    team_name_map = teams_df.set_index('id')['name'].to_dict()
    ...
    players_df['team_name'] = players_df['team'].map(team_name_map) # <-- ADDED: Map Team Name
    ...
    # This weights the most recent Gameweek highest (45%)
    ...
    # Final Projection = Base Form * Fixture Difficulty Adjustment
    ...
    # Reviewing the forecasting methodology:
    # 1. Base Form (Weighted recent points)
    # 2. Fixture Multiplier (Difficulty factor) 
    # 3. Final Projection (Base Form * Multiplier)
